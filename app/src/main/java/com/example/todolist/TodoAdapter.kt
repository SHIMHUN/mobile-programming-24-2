...
//            }
            binding.root.setOnClickListener {
                ...
                AlertDialog.Builder(binding.root.context)
                    ...
                    .setPositiveButton("수정완료") { dialog, _ ->
                        CoroutineScope(Dispatchers.IO).launch {
                            // Room에서 데이터를 가져옴
                            val innerLstTodo = roomDatabase.todoDao().getAllReadData()
                            for (item in innerLstTodo) {
                                if (item.todoContent == todoItem.todoContent && item.todoDate == todoItem.todoDate) {
                                    // Room DB에 업데이트할 새 객체 생성
                                    val updatedItem = item.copy(
                                        todoContent = bindingDialog.etMemo.text.toString(),
                                        todoDate = SimpleDateFormat("yyyy-MM-dd").format(Date())
                                    )

                                    // 데이터베이스 업데이트
                                    roomDatabase.todoDao().updateTodoDate(updatedItem)
                                }
                            }
                            ...
                            lstTodo[adapterPosition] = updatedTodoItem
